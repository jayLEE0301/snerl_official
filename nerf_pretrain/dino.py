...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        new_dest = descriptors.view(1, 31, 31, 384)
        new_dest = torch.mean(new_dest, axis=3).detach().cpu().numpy()
        import numpy as np
        new_dest = (new_dest - np.min(new_dest))/(np.max(new_dest)-np.min(new_dest))
        import matplotlib.pyplot as plt
        plt.imsave('test.png', new_dest[0])
