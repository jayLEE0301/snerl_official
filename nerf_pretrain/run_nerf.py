...
def batchify_rays(rays_flat, chunk=1024*32, latent=None, **kwargs):
    """Render rays in smaller minibatches to avoid OOM.
    """
    all_ret = {}
    for i in range(0, rays_flat.shape[0], chunk):
        ret = render_rays(rays_flat[i:i+chunk], latent=latent, **kwargs)
        for k in ret:
            if k not in all_ret:
                all_ret[k] = []
...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
                  near=0., far=1.,
                  use_viewdirs=False, c2w_staticcam=None, latent=None,
                  **kwargs):
    """Render rays
    Args:
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, chunk, latent=latent, **kwargs)
    for k in all_ret:
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, latent=None):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        t = time.time()
        rgb, disp, acc, semantic, dino, _ = render(H, W, K, chunk=chunk, c2w=c2w[:3,:4], latent=latent, **render_kwargs)
...
def create_nerf(args):
    ...
    latent_embed = PixelEncoder().to(device)
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    semantic_map = torch.sum(weights[...,None].detach() * semantic, -2)  # [N_rays, 3]
...
#     raw = run_network(pts)
    ...
    rgb_map, disp_map, acc_map, weights, depth_map, semantic_map, dino_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)

    if N_importance > 0:

        rgb_map_0, disp_map_0, acc_map_0, semantic_map_0, dino_map_0 = rgb_map, disp_map, acc_map, semantic_map, dino_map
...
#         raw = run_network(pts, fn=run_fn)
    ...
    ret = {'rgb_map' : rgb_map, 'disp_map' : disp_map, 'acc_map' : acc_map, 'semantic_map': semantic_map, 'dino_map' : dino_map}
...
def train():
    ...
    if args.dataset_type == 'blender':
        images, poses, render_poses, hwf, i_split, semantics = load_blender_data(args.datadir, args.half_res, args.testskip)
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, latent_embed = create_nerf(args)
    global_step = start
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            time_index = np.random.choice(np.arange(len(images[0])))
            ...
            with torch.no_grad():
                ...
                target_dino = dino_extractor.extract_descriptors(target_dino.to(device).unsqueeze(dim=0).permute(0,3,1,2), 11, 'key', False).view(1, 31, 31, 384)
                target_dino = F.interpolate(target_dino.permute(0,3,1,2), (128, 128), mode='bilinear').permute(0,2,3,1)[0]
                
            target_semantic = local_semantics[img_i]
            target_semantic = torch.Tensor(target_semantic).long().to(device)
            ...
            if N_rand is not None:
                ...
                target_semantic_s = target_semantic[select_coords[:, 0], select_coords[:, 1]]
                target_dino_s = target_dino[select_coords[:, 0], select_coords[:, 1]] 
                
        
        #####  Core optimization loop  #####
        rgb, disp, acc, semantic, dino, extras = render(H, W, K, chunk=args.chunk, rays=batch_rays,
                                                verbose=i < 10, retraw=True, latent=latent,
                                                **render_kwargs_train)
        ...
        if 'rgb0' in extras:
            ...
            semantic_loss0 = nn.CrossEntropyLoss()(extras['semantic0'], target_semantic_s)
        ...
        if i%args.i_video==0 and i > 0:
            ...
            with torch.no_grad():
                rgbs, disps = render_path(render_poses, hwf, K, args.chunk, render_kwargs_test, latent=latent)
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            with torch.no_grad():
                render_path(test_poses, hwf, K, args.chunk, render_kwargs_test, savedir=testsavedir, latent=latent)
